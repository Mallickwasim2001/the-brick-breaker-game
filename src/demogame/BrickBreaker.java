...
public class BrickBreaker extends JPanel implements KeyListener, ActionListener, Runnable {
	...
	public void run() {
		while (RUNNING) {
			...
			for (int i = 0; i < Brick.length; i++) {
				if (Brick[i] != null) {
					if (Brick[i].intersects(Ball)) { // if ball intersects with brick 
						Brick[i] = null;//then brick will disappear
						// movex = -movex;
						movey = -movey;
						count++;
					} // end of 2nd if..
	...
	// HANDLING KEY EVENTS
	//@override means its an annotation that informs the compiler that the element is meant to override an element declared in a super class.
	@Override
...
//		Bat = new Rectangle(160, 245, 40, 5);
		...
		// Creating bricks for the game, with size width = 80, height = 60
		createBricks(80, 60);
		createBricks(70, 50);
